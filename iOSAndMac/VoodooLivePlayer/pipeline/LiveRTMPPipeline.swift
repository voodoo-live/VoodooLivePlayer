...
class LiveRTMPPipeline : LiveCustomPipeline {
    var loader: LiveLoaderProtocol
    var demuxer: LiveDemuxer
    var address: RTMPAddress
    let dispatchQueue = DispatchQueue(label: "VoodooLivePlayer.LiveRTMPPipeline.dispatchQueue")

    init?(player: LivePlayer, source:LiveStreamSource) {
        if source.type != .RTMP { return nil }
        if let address = RTMPAddress(url: source.url) {
            self.address = address
        } else {
            return nil
        }
        player.playerViewController.mode = .sampleBufferMode
        self.loader = LiveRTMPLoader(address: self.address)
        self.demuxer = LiveRTMPDemuxer()
